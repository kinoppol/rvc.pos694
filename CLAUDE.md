...
- **Platform admin console** (`/admin/merchants`, `/admin/settings`) — gated by `PlatformAdminMiddleware` (session user must have `is_platform` + `owner` role, i.e. belong to the `merchants.is_platform = 1` tenant created by `install.php`). `GET /admin/merchants/{id}` is a per-merchant overview (branch/staff/product counts, per-branch revenue, 14-day activity, charts) — the dataset comes from `App\Services\MerchantOverview` and the same view backs the owner's own `GET /store/overview`. Approve/suspend merchants and **impersonate** them (`POST /admin/merchants/{id}/impersonate` swaps the session tenant; `POST /admin/impersonate/stop` restores it — that stop route sits outside the platform-admin group on purpose).
