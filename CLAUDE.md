...
- **Merchant self-registration** (`/register`, public) — creates a `merchants` row + owner user; lands `pending` when `system_settings.require_approval = 1`, else `active`. **Store-group join:** if the applicant enters a `merchants.join_code` (an HQ merchant generates/revokes it at `/store`, `POST /store/join-code[/disable]`), no new merchant is created — instead a new `branches` row is added under that merchant and the applicant becomes its `manager`. `require_approval` is bypassed for this path: holding the code is the authorization. An **already-registered** merchant can also join later — `POST /store/join` (owner only) re-parents every `merchant_id`-scoped row (`branches`, `users`, `categories`, `products`, `stock_transfers`, `member_tiers`, `members`, `coupons`, `sales`) to the target, demotes the source owner(s) to `manager`, deletes the source `merchants` row, and drops the session (irreversible).
- **Platform admin console** (`/admin/merchants`, `/admin/settings`) — gated by `PlatformAdminMiddleware` (session user must have `is_platform` + `owner` role, i.e. belong to the `merchants.is_platform = 1` tenant created by `install.php`). `GET /admin/merchants/{id}` is a per-merchant overview (branch/staff/product counts, per-branch revenue, 14-day activity, charts) — the dataset comes from `App\Services\MerchantOverview` (also: this-month top sellers / top products, and a rolling-12-month sales total vs the ฿1.8M VAT-registration threshold) and the same view backs the owner's own `GET /store/overview`. Approve/suspend merchants and **impersonate** them (`POST /admin/merchants/{id}/impersonate` swaps the session tenant; `POST /admin/impersonate/stop` restores it — that stop route sits outside the platform-admin group on purpose).
